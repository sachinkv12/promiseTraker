...
const userRole = user.userRole;
    ...
    if (userRole === Roles.SUPER_ADMIN) {
      ...
      if (req.method === 'GET' || req.method === 'POST') {
        ...
        return res.status(403).json({ message: 'Access not allowed as userRole = 2' });
      }
      ...
      if (req.method === 'GET') {
        ...
        return res.status(403).json({ message: 'Access not allowed as userRole = 3' });
      }
    } else {
      return res.status(403).json({ message: 'Access not allowed for other userRoles or unauthorized users' });
    }
  } catch (error) {
